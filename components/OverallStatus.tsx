...
export default function OverallStatus({
  ...
  let statusString = ''
  let icon = <IconAlertCircle style={{ width: 64, height: 64, color: 'var(--status-red)' }} />
  if (state.overallUp === 0 && state.overallDown === 0) {
    ...
    icon = <IconCircleCheck style={{ width: 64, height: 64, color: 'var(--status-green)' }} />
  } else {
  ...
  return (
    <Container size="md" mt="xl" className="overall-status-hero">
      <Center>{icon}</Center>
      <Title mt="sm" style={{ textAlign: 'center', fontWeight: 800, letterSpacing: '-0.03em' }} order={1}>
        {statusString}
      </Title>
      <Title mt="sm" style={{ textAlign: 'center', color: 'var(--text-secondary)', fontWeight: 500 }} order={5}>
        {t('Last updated on', {
