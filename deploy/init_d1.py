...
if not r['success']:
    ...
    if r['errors'][0]['code'] == 7502:
        ...
        print("Authentication warning when creating D1 database. Proceeding to check if database already exists.")
    else:
        print("Unknown error creating D1 database: ", r)
        print("Proceeding to check if database already exists.")
else:
...
if not r['success']:
    print("Warning: Error initializing D1 database. Proceeding since database/tables may already be initialized.")
else:
