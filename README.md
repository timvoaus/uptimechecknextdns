...
This deployment expects Telegram credentials to be stored directly as Cloudflare Worker secrets on the Worker named `uptimeflare_worker`.
...
Set or rotate them with Wrangler:

```bash
npx wrangler secret put TELEGRAM_BOT_TOKEN --name uptimeflare_worker
npx wrangler secret put TELEGRAM_CHAT_ID --name uptimeflare_worker
```

`TELEGRAM_BOT_TOKEN` should contain only the token value from BotFather, for example `123456789:ABC...`; do not include the `bot` prefix. `TELEGRAM_CHAT_ID` should contain the numeric chat ID.

These secrets are not read from GitHub repository secrets during deployment. After updating them directly on Cloudflare, the next scheduled check will use the new values.
